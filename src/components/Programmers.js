...
export default function Programmers() {
  ...
  const [listNames, setListNames] = useState(listOfAwesome)
  const [featured, getFeatured] = useState(0)
  
  const getNameOfFeatured = () => {
    return listNames[featured-1].name
    // Leave this for last!
      ...
      <div className='programmers'>
        {
          ...
          listNames.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { getFeatured(dev.id)/* in here set the featured id to be dev.id */ }}>Feature</button>
            </div>
          )
        }
      </div>
