...
const [mood, setMood] = useState(initialMood)
  const makeHappy = () => {
    /* STEP 4 */
    setMood(mood => mood.replace(mood == initialMood ? initialMood : sadMood, happyMood));
  };
  const makeSad = () => {
    /* STEP 5 */
    setMood(mood => mood.replace(mood == initialMood ? initialMood : happyMood, sadMood))
  };
  const reset = () => {
    /* STEP 6 */
    setMood(mood => mood.replace(mood == happyMood ? happyMood : sadMood, initialMood) )
  };
